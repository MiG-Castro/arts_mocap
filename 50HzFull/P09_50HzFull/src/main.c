...
static uint32_t re_sync_k = 10, temp_k = 0, iso_seq_n = 0;
...
static uint64_t fs_hz, ts_us;
static uint32_t ts_ticks, ts_first_tick, ts_resync, now_ticks, iso_ts_us, stop_pkt, counter_tx, ts_delta = 8000;
static uint8_t units, samples_x_pkt = 3, t_k = 0, n_pkt = 0;
...
# define SAMPLE_SIZE 20 // 12 Quat + 6 Acc + 6 Gyr = 20
# define N_PKT_SIZE  4  // no pkt (uint_32)
...
static void sensor_work_handler(struct k_work *work)
{
	...
	static uint8_t indx_pos, temporal_sample_counter, n_pkt_size, samp_size;
	...
	*/
	...
	// En promedio la lectura a 100Khz I2C tarda 5ms   (max 6.9ms)
	// En promedio la lectura a 400Khz I2C tarda 3.8ms (max 6.5ms)
	if (bno_ok) {
		// t1 = k_cycle_get_32();
	...
	// Opcion 3: Comentar opcion 1 & 2 y enviar 0s
	...
	if (((temporal_sample_counter + 1) % samples_x_pkt) == 0) {
		...
		memcpy(packet_in_progress, &n_pkt, n_pkt_size);
