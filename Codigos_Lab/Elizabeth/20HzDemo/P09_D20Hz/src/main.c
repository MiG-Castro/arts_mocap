...
// Necesario para TASKS_CONSTLAT --- Agregado Eli ---
#include <hal/nrf_power.h>
// Necesario para verificar estado del reloj
#include <hal/nrf_clock.h>  
...
static const bt_addr_le_t target_addr = {
	...
	.a = {.val = {0xB3, 0x1C, 0x96, 0xB9, 0xCC, 0xE5}} // eli
};
...
static uint32_t re_sync_k = 200, temp_k = 0, iso_seq_n = 0;
...
// //Probando timer rtc ------------Eli-------------------------------------------
// const struct device *hw_timer_dev = DEVICE_DT_GET(DT_NODELABEL(rtc2));
...
{
	static uint8_t packet_in_progress[PACKET_SIZE] = {1, 0, 0, 0};
...
static void iso_recv(struct bt_iso_chan *chan, const struct bt_iso_recv_info *info, struct net_buf *buf)
{
	...
	if (!sync_ok) {
		...
		n_pkt = iso_seq_n * 4;
		LOG_INF("num Packet to first sync %d numP %d", iso_seq_n, n_pkt);
	...
	// }
		...
		if ((iso_seq_n % re_sync_k) == 0) { // || temp_k >= re_sync_k
			...
			LOG_INF("alarm_cfg0.ticks resync: %d", alarm_cfg0.ticks);
			counter_set_channel_alarm(hw_timer_dev, 0, &alarm_cfg0);
			temp_k ++;

			// Resync no. pkt
			memcpy(&iso_seq_n, buf->data, 4);
			LOG_INF("num Packet before to Re sync %d", n_pkt);
			n_pkt = iso_seq_n * 4;
			LOG_INF("num Packet to Re sync %d numP %d", iso_seq_n, n_pkt);

			// // codigo para desabilitar la resincronizacion ISO
			// if (temp_k > 20){
			// 	k_sem_give(&sem_sync_lost);
			// }
		}
...
// Force sub-power mode Constant Latency --- Agregado Eli ----
void force_high_performance_mode(void)
{
    // Obtener el dispositivo de reloj del sistema
    const struct device *clock = DEVICE_DT_GET_ONE(nordic_nrf_clock);

    if (!device_is_ready(clock)) {
        printk("Error: Dispositivo de reloj no listo\n");
        return;
    }

    // SOLICITAR EL HFXO (Cristal)
    int err = clock_control_on(clock, (void *)CLOCK_CONTROL_NRF_SUBSYS_HF);
    if (err) {
        printk("Aviso: No se pudo solicitar HFXO via API: %d\n", err);
    }
	
    // ESPERAR A QUE EL HFXO ESTÉ ESTABLE
    // Miramos directamente el registro del hardware para estar 100% seguros
    // de que el cristal está oscilando antes de continuar.
    // NRF_CLOCK_HFCLK_HIGH_ACCURACY = Cristal Externo.
    printk("Esperando estabilizacion del cristal (HFXO)...\n");
    while (!nrf_clock_hf_is_running(NRF_CLOCK, NRF_CLOCK_HFCLK_HIGH_ACCURACY)) {
        k_msleep(1);
    }
    printk("HFXO estable y corriendo.\n");

    // FORZAR LATENCIA CONSTANTE
    // Esto escribe un '1' en el registro TASKS_CONSTLAT.
    // Evita que el regulador interno baje a modo bajo consumo, eliminando
    // el tiempo de retraso al despertar la CPU o los periféricos.
    nrf_power_task_trigger(NRF_POWER, NRF_POWER_TASK_CONSTLAT);
    printk("Modo Constant Latency activado.\n");
}
...
int main(void)
{
	...
	// Forzar Constant Latency mode
    force_high_performance_mode();
	...
	bool terminate_sync = false;
	while (!terminate_sync) {
		// Reset all
		gpio_pin_set_dt(&red_led, 1);
		k_msleep(500);
		...
		// // agregado eli para cerrar recivo ISO luego de sincronizacion
		// k_msleep(100);
		// terminate_sync = true;
