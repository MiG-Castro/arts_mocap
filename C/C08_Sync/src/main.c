 ...
 * - Central hace el primer Write en la funcion de descubrimiento de servicios
 * - Rutina de syncronizacion de callback de ACK
*/
...
static uint16_t conn_interv_N = 20, latency_N = 0, timeout_N = 100;
...
static bool first_p = false, sync_pn[MAX_CONNECTIONS] = {0};
static int64_t t_ref, t_ack_us, tpn[MAX_CONNECTIONS] = {0};
static uint32_t cycles;

static uint8_t samples_x_pkt = 3;
static uint16_t fs_hz = 60, start_delay_ms = 50;
static uint32_t ts_us, start_delay_us, n_sample, tol_us = 600, delta_us = 3000;
...
void on_write_completed(struct bt_conn *conn, uint8_t err, struct bt_gatt_write_params *params)
{
    ...
    // if the peripheral is not sync
    uint32_t ci_us = conn_interv_N * 1250;
    if (!sync_pn[idx]) {
        struct bt_imus_client *imus_client = bt_conn_ctx_get(&conns_ctx_lib, conn);
        
        // First response -> Save time & Send command to sync
        if(tpn[idx] == 0) {
            t_ack_us = k_cyc_to_us_floor64(cycles);
            tpn[idx] = t_ack_us;
            ...
            if (!first_p) {
                // if is the first peripheral
                n_sample = 1;
                start_delay_us = ci_us + ts_us - delta_us; // To star in sample 1
            } else {
                // Sync with P0!!!

                // What is the next next sample that P0 will take?
                // 1. Calculate n (the next sample)
                n_sample = (t_ack_us + delta_us + ci_us - t_ref + ts_us - 1) / ts_us;
                n_sample++; // To start in the next next sample
                // 2. Calculate the time to start in sample n + 1
                start_delay_us = t_ref + ci_us - delta_us + (n_sample * ts_us);
            }

            // Build the cmd pkt
            cmd_tx[0] = CMD_SYNC_START;
            memcpy(&cmd_tx[1], &start_delay_us, sizeof(start_delay_us));
            // memcpy(&cmd_tx[1], &fs_hz, sizeof(fs_hz));
            // memcpy(&cmd_tx[3], &start_delay_us, sizeof(start_delay_us));
            // memcpy(&cmd_tx[7], &samples_x_pkt, sizeof(samples_x_pkt));
            // memcpy(&cmd_tx[8], &n_sample, sizeof(n_sample));
            // Send
            my_imus_write_command(conn, imus_client, cmd_tx, sizeof(cmd_tx), on_write_completed);
        } else {

            // Second response
            if (k_cyc_to_us_floor64(cycles) - tpn[idx] < tol_us + 2 * ci_us) {
                ...
                // The process took more than 2 CI -> Retry
                tpn[idx] = 0;
                my_imus_write_command(conn, imus_client, &(const uint8_t){0x28}, 1, on_write_completed);
                printk("P[%d] Faild Syc -> Retry\n", idx);
            }
        }
    } 
...
static uint8_t on_gatt_notify(struct bt_conn *conn, struct bt_gatt_subscribe_params *params, const void *data, uint16_t length)
{
    ...
    LOG_INF("Rx: P%d[%u] %u", idx, buffer[0], length);
...
static void discovery_complete(struct bt_gatt_dm *dm, void *context)
{
    ...
    // Comenzamos el proceso de sincronizacion
    LOG_INF("P[%d]-SyncW", idx);
    my_imus_write_command(conn, imus_client, &(const uint8_t){0x28}, 1, on_write_completed);
            ...
            if (len >= 2) {
                ...
                if (payload[0] == 0xFF) {
                    for (size_t i = 0; i < MAX_CONNECTIONS; i++) {
                        const struct bt_conn_ctx *ctx = bt_conn_ctx_get_by_id(&conns_ctx_lib, i);
                        if (ctx) {
                            int eer = my_imus_write_command(ctx->conn, ctx->data, &ble_cmd, 1, on_write_completed);
                            LOG_INF("TxP%d-%02X, %d", i, ble_cmd, eer);
                            bt_conn_ctx_release(&conns_ctx_lib, ctx->data);
                        }
                    }
                } else {
            ...
            switch (pkt_builder.state) {
                ...
                case STATE_READ_END:
                    if (c == PKT_END_BYTE) {
                        ...
                        printk("CMD-Rx: 0x%02X, len %u\n", pkt_builder.type, pkt_builder.len);
                    } else {
