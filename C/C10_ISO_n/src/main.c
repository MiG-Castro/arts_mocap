 ...
 * Comunicacion UART -> ACTUALIZADA (Uso de USB CDC)
 * - Hilo para Rx & Hilo para Tx.
...
static uint16_t scan_window_N = 16, scan_interval_N = 160; // u = 0.625ms
...
static uint16_t conn_interv_N = 8, latency_N = 3, timeout_N = 100;
// Values to set in the data length
static uint16_t data_length = 70, tx_time_us = 800; // DL = desired value + 7B of headers
...
// Direcciones de TODOS los dispositivos permitidos
// DEBEN DE ESTAR EN LITTLE ENDIAN!!!
// Original (BE) FA:53:60:43:10:2F -> LE=0x2F, 0x10, 0x43, 0x60, 0x53, 0xFA
static const bt_addr_le_t sensor_wl[] = {
    // {.type = BT_ADDR_LE_RANDOM, .a = { .val = {0x, 0x, 0x, 0x, 0x, 0x}}},
    {.type = BT_ADDR_LE_RANDOM, .a = { .val = {0x0A, 0x77, 0x77, 0x2A, 0x5C, 0xFB}}},
    {.type = BT_ADDR_LE_RANDOM, .a = { .val = {0x84, 0x20, 0x1F, 0x32, 0x67, 0xDC}}},
    {.type = BT_ADDR_LE_RANDOM, .a = { .val = {0x1B, 0xF8, 0xB5, 0x7C, 0xB0, 0xC0}}},
    {.type = BT_ADDR_LE_RANDOM, .a = { .val = {0xA4, 0x27, 0xE7, 0x61, 0x19, 0xD3}}},
    {.type = BT_ADDR_LE_RANDOM, .a = { .val = {0xD5, 0xD2, 0x07, 0x3E, 0x34, 0xE6}}},
...
// static uint8_t samples_x_pkt = 3, fs_hz = 60, pkts_x_iso = 4;
...
NET_BUF_POOL_FIXED_DEFINE(bis_tx_pool, BIS_ISO_CHAN_COUNT, BT_ISO_SDU_BUF_SIZE(CONFIG_BT_ISO_TX_MTU),
...
static uint8_t on_gatt_notify(struct bt_conn *conn, struct bt_gatt_subscribe_params *params, const void *data, uint16_t length)
{
    ...
    if (length > 0) {
        ...
        if (params->value_handle == imus_client->sensordata_handle) {
            if (print_log) printk("RX_N:P%d[%u-%uB]\n", idx, packet_num, length);
        }

        if (params->value_handle == imus_client->exercisedetection_handle) {
            if (print_log) printk("RX_I:P%d[%u-%uB]\n", idx, packet_num, length);
        }
...
static void on_conn_param_updated(struct bt_conn *conn, uint16_t interval, uint16_t latency, uint16_t timeout)
{
    ...
    if (err < 0 && print_log) LOG_ERR("DL request failed (err %d).", err);
}
...
static void connected(struct bt_conn *conn, uint8_t hci_err)
{
    ...
    if (!imus_client) {
        if (print_log) LOG_ERR("There is no memory for context.");
...
static void interrupt_handler(const struct device *dev, void *user_data)
{
    ...
    while (uart_irq_update(dev) && uart_irq_is_pending(dev)) {
        ...
        if (uart_irq_tx_ready(dev)) {
            ...
            if (sent < rb_len) {
                ring_buf_put(&ringbuf, buffer + sent, rb_len - sent);
            }
        }
        // RX
        if (uart_irq_rx_ready(dev)) {
            ...
            if (recv_len > 0) {
                ring_buf_put(&rx_ringbuf, buf, recv_len);
                k_sem_give(&rx_sem);
            }
        }
    }
...
void send_uart_packet(uint8_t msg_type, uint8_t p_idx, const uint8_t *payload, uint16_t len)
{
    ...
    static uint8_t encoded[UART_TX_BUF_SIZE * 2 + 2]; // peor caso: todos los bytes escapados
    if (len + 3 > UART_TX_BUF_SIZE) return;
    uint8_t lnn = len + 3;
    tx_buf[0] = len; // REVISAR SI PONER EL LEN COMPLETO O SOLO LO DEL PKT RECIBIDO!!!
    tx_buf[1] = msg_type;
    tx_buf[2] = p_idx;
    memcpy(&tx_buf[3], payload, len);
    size_t enc_len = uart_frame_encode(encoded, sizeof(encoded), tx_buf, lnn);
    if (enc_len == 0) return;
    uint32_t written = ring_buf_put(&ringbuf, encoded, enc_len);
    if (written < enc_len) {if (print_log) printk("[C] WRN - Bytes losses in TX!!!.\n");}
...
static void uart_rx_thread(void)
{
    ...
    while (true) {
        // Espera hasta que ISR deposite bytes
        k_sem_take(&rx_sem, K_FOREVER);
        // Drena todo lo disponible en el ring buffer RX
        while (ring_buf_get(&rx_ringbuf, &c, 1)) {
            // Start of the pkt
            if (c == PKT_START_BYTE) {
                pos = 0;
                escape_next = false;
...
int main(void)
{
    ...
    // ===================================================================
    // Init ring buffer
    ring_buf_init(&ringbuf, sizeof(ring_buffer), ring_buffer); //TX
    ring_buf_init(&rx_ringbuf, sizeof(rx_ring_buffer), rx_ring_buffer); //RX
    
    uart_irq_callback_set(uart_dev, interrupt_handler);
    // NO uart_irq_rx_enable aquí
    ...
    // // wait for the line control signal (optional)
    // uint32_t dtr = 0;
    // while (true) {
    ...
    printk("%s %s. CEL=%dus, ISO_Interval=%dms\n", 
    ...
    while (true) {
        ...
        if (bt_iso_chan_send(&bis_iso_chan[0], buf, 0) < 0) {
