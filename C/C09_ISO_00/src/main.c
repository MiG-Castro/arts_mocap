 ...
 * 
 * Adicion de rutina de sincronizacion por BIG
*/
#include <stdlib.h>
...
// To the central
#define CMD_TYPE_DATOS_SIM_ON 0x14  // ACTIVA LA TRANSMISION DE DATOS SIMULADOS (NO PKT + 0'S)
#define CMD_TYPE_DATOS_SIM_OFF 0x15  // DESACTIVA LA TRANSMISION DE DATOS SIMULADOS (NO PKT + 0'S)
...
static void handle_uart_command(const uint8_t *pkt, uint8_t lnn)
{
    ...
    switch (cmd_type) {
        ...
        case CMD_TYPE_DATOS_SIM_ON:
            tx_simulado = true;
            if (print_log) printk("[C] Tx_Simulado ON.\n");
            break;

        case CMD_TYPE_DATOS_SIM_OFF:
            tx_simulado = false;
            if (print_log) printk("[C]Tx_Simulado OFF.\n");
            break;
...
int main(void)
{
    uint8_t sensor_simulado[20] = {0};
    uint32_t num_pkt_simulado = 0;
    struct uart_packet pkt_simulado = {
        .msg_type = MSG_TYPE_SENSOR_DATA,
        .p_idx = 1,
        .len = 20
    };
    ...
    while (true) {
        ...
        if (tx_simulado) {
            num_pkt_simulado++;
            memcpy(sensor_simulado, &num_pkt_simulado, 4);
            memcpy(pkt_simulado.payload, sensor_simulado, 20);
            k_msgq_put(&uart_tx_queue, &pkt_simulado, K_NO_WAIT);
        }
    }
}