 ...
 *   - Comandos & actualizacion de parametros
 * - Funcion de uart_tx
 */
...
static uint16_t data_length = 251, tx_time = 2120;
...
{LOG_INF("Comando enviado con exito");}
...
static uint8_t on_gatt_notify(struct bt_conn *conn, struct bt_gatt_subscribe_params *params, const void *data, uint16_t length)
{
    ...
    if (params->value_handle == imus_client->sensordata_handle) {
        ...
        LOG_INF("[Periférico %d] Rx Notify pkt #%u", idx, packet_num);
        send_uart_packet(MSG_TYPE_SENSOR_DATA, (uint8_t)idx, data, length);
    } else if (params->value_handle == imus_client->exercisedetection_handle) {
        // CORRECCIÓN: Imprimir el índice en un log separado.
        LOG_INF("[Periférico %d] Rx indicate", idx);
        send_uart_packet(MSG_TYPE_EVENT_DATA, (uint8_t)idx, data, length);
    }
...
static void discovery_complete(struct bt_gatt_dm *dm, void *context)
{
    ...
    LOG_INF("[Periférico %d] Suscipcion completa", idx);
...
static void exchange_mtu_cb(struct bt_conn *conn, uint8_t err, struct bt_gatt_exchange_params *params)
{
	...
	LOG_INF("MTU set to %d bytes", bt_gatt_get_mtu(conn) - 3);
    ...
    if (gatt_err) LOG_ERR("Fallo al iniciar el descubrimiento (err %d)", gatt_err);
    bt_conn_ctx_release(&conns_ctx_lib, imus_client);
}

static void on_data_len_updated(struct bt_conn *conn, struct bt_conn_le_data_len_info *info)
{
	LOG_INF("Data Length (Tx/Rx): %u/%u bytes, %u/%u us", 
    ...
    if (err) LOG_ERR("Fallo en la petición de intercambio de MTU (err %d)", err);
}
...
static void on_conn_param_updated(struct bt_conn *conn, uint16_t interval, uint16_t latency, uint16_t timeout)
{
	...
	LOG_INF("Connection parameters set to %.2f ms, %d, %d ms", ci, latency, sto);
    ...
    if (err) LOG_ERR("Fallo en la petición de DLE (err %d)", err);
}

static void on_phy_updated(struct bt_conn *conn, struct bt_conn_le_phy_info *param)
{
    if (param->tx_phy == BT_CONN_LE_TX_POWER_PHY_1M) LOG_INF("PHY set to 1M");
    if (param->tx_phy == BT_CONN_LE_TX_POWER_PHY_2M) LOG_INF("PHY set to 2M");
    ...
    if (err) LOG_ERR("Fallo en la petición de parámetros de conexión (err %d)", err);
}
...
static void connected(struct bt_conn *conn, uint8_t hci_err)
{
    ...
    if (err) LOG_ERR("Fallo en la petición de PHY (err %d)", err);
}
...
static void disconnected(struct bt_conn *conn, uint8_t reason)
{
    ...
    int idx = get_conn_index(conn);
    LOG_INF("Periferico %d - %s, desconectado (razón 0x%02x)", idx, addr, reason);
...
void send_uart_packet(uint8_t msg_type, uint8_t p_idx, const uint8_t *payload, uint16_t len)
{
    ...
    if (len + 6 > UART_TX_BUF_SIZE) {
        LOG_ERR("Paquete UART demasiado grande para enviar (%u bytes)", len);
    } else {
...
static void handle_uart_command(uint8_t cmd_type, const uint8_t *payload, uint8_t len)
{
    switch (cmd_type) {
        case CMD_TYPE_START_SCAN:
            LOG_INF("UART CMD: Iniciar escaneo");
            bt_scan_start(BT_SCAN_TYPE_SCAN_ACTIVE);
            scan_on = true;
            break;

        case CMD_TYPE_STOP_SCAN:
            LOG_INF("UART CMD: Detener escaneo");
            bt_scan_stop();
            scan_on = false;
            break;
            ...
            case CMD_TYPE_DISCONNECT:
                ...
                if (idx == 0xFF) {
                    LOG_INF("UART CMD: Desconectar TODOS");
                    for (size_t i = 0; i < MAX_CONNECTIONS; i++) {
                        const struct bt_conn_ctx *ctx = bt_conn_ctx_get_by_id(&conns_ctx_lib, i);
                        if (ctx) {
                    ...
                    LOG_INF("UART CMD: Desconectar %d", idx);
        ...
        case CMD_UPDATE_PARAMS:
            if (len == 11) {
                ...
                // Connection Interval
                uint16_t n = sys_get_le16(&payload[1]);
                if (n >= 6 &&  n <= 3200) conn_interv = n;
                ...
                if (n >= cal_tout) timeout_ms = n;
                else timeout_ms = cal_tout;
                ...
                if (n >= 328 &&  n <= 17040) tx_time = n;

                LOG_INF("Parameters Update: PHY=%s, CI=%.2fms (%u), L=%u, Sto=%ums, DL=%u, TxRx=%uus",
        ...
        case CMD_TYPE_SEND_CMD:
            if (len >= 2) {
                uint8_t p_idx = payload[0];
                uint8_t ble_cmd = payload[1];
                LOG_INF("UART CMD: Enviar comando 0x%02X a periférico %u", ble_cmd, p_idx);
            ...
            break;
        default:
            LOG_WRN("Comando UART desconocido: 0x%02X", cmd_type);
            break;
    }
}
        ...
        switch (state) {
            ...
            case STATE_READ_LEN:
                ...
                if (len + 3 > UART_RX_BUF_SIZE) {
                    LOG_WRN("UART: Longitud de payload inválida (%u). Descartando.", len);
            ...
            case STATE_READ_TYPE:
                ...
                if (len == 0) state = STATE_READ_CHECKSUM;
            ...
            case STATE_READ_PAYLOAD:
                //Post incremento, se ejecuta el valor actual y despues del acceso al array se incrementa pos
                buffer[pos++] = c;
                if (pos > len) state = STATE_READ_CHECKSUM;
                break;
            ...
            case STATE_READ_CHECKSUM:
                {
                    ...
                    if (received_checksum == calculated_checksum) {
                        ...
                        LOG_WRN("UART: Checksum incorrecto.");
                        state = STATE_WAIT_START;
                    }
                }
                break;
            ...
            case STATE_READ_END:
                if (c == PKT_END_BYTE) {
                    ...
                    LOG_WRN("UART: Byte de fin incorrecto.");
                    // state = STATE_WAIT_START;
                }
...
{
    int err;
    LOG_INF("Central - Dongle");

    if (!device_is_ready(uart_dev)) {
        LOG_ERR("Dispositivo UART no encontrado!");
        return 0;
    }
    ...
    LOG_INF("Inicializacion completa.");
    return 0;
}