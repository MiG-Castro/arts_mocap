 ...
 * - Funcion de uart_tx
 * Pruebas de tiempo de Rx-pkts
 * - Retransmisiones ciclicas
 * - Conteo de tiempo con LOGs y/o k_uptime_get (ms)
 */
...
static uint16_t conn_interv = 10, latency = 0, timeout_ms = 1000;
static uint16_t data_length = 73, tx_time = 800;
...
static void connected(struct bt_conn *conn, uint8_t hci_err)
{
    ...
    if (print_log) printk("[] Connected to: %s.\n", addr);
...
static void disconnected(struct bt_conn *conn, uint8_t reason)
{
    ...
    int idx = get_conn_index(conn);
    if (print_log) printk("[] Peripheral %d - %s disconnected (0x%02x).\n", idx, addr, reason);
...
static void handle_uart_command(uint8_t cmd_type, const uint8_t *payload, uint8_t len)
{
    ...
    switch (cmd_type) {
        case CMD_TYPE_TX_UART:
            send_uart = !send_uart;
            if (print_log) printk("[] UART Send Recieved Pkts -> %s.\n", send_uart ? "ON" : "OFF");
            break;
        ...
        case CMD_TYPE_START_SCAN:
            if (print_log) printk("[] Start Scan.\n");
            bt_scan_start(BT_SCAN_TYPE_SCAN_ACTIVE);
            scan_on = true;
            break;

        case CMD_TYPE_STOP_SCAN:
            if (print_log) printk("[] Scan Stop.\n");
            bt_scan_stop();
            scan_on = false;
            break;
            ...
            case CMD_TYPE_DISCONNECT:
                p_idx = payload[0];
                if (p_idx == 0xFF) {
                    if (print_log) printk("[] Disconnect ALL Peripherals.\n");
                    for (size_t i = 0; i < MAX_CONNECTIONS; i++) {
                        const struct bt_conn_ctx *ctx = bt_conn_ctx_get_by_id(&conns_ctx_lib, i);
                        if (ctx) {
                    ...
                    if (print_log) printk("[] Disconnect [Peripheral %d].\n", p_idx);
        ...
        case CMD_UPDATE_PARAMS:
            if (len == 11) {
                ...
                if (print_log) printk("[] Update PHY=%s, CI=%.2fms (%u), L=%u, STO=%ums, DL=%u, T=%uus.\n",
        ...
        case CMD_TYPE_SEND_CMD:
            if (len >= 2) {
                ...
                if (print_log) printk("[] Sending CMD 0x%02X to %u.\n", ble_cmd, p_idx);
        ...
        case CMD_UPDATE_TIMER:
            if (len >= 5) {
                ...
                if (print_log) printk("[] Sending CMD 0x%02X -> %d Hz, %d ms to %u.\n", timer[0],n, k, p_idx);
        ...
        case CMD_STOP_PKT:
            if (len >= 5) {
                ...
                if (print_log) printk("[] Sending CMD 0xBB -> StopPkt %d to %u.\n", pkt, p_idx);
...
static void uart_rx_thread(void)
{
    ...
    while (true) {
        ...
        if (uart_poll_in(uart_dev, &c)) {
            k_sleep(K_MSEC(1000));
            // printk("esperando comando...\n");
            // DEBE DE HABER UN INTERMEDIARIO 
            // - EL FIFO ES DE SOLO 6 Bytes pero el delay de 1s + envio de +6Bytes NO LE AFECTA!!!
            continue;
        }
...
int main(void)
{
    ...
    struct bt_le_conn_param *sc_conn_param = BT_LE_CONN_PARAM(
        conn_interv, conn_interv, latency, timeout_ms / 10);
    struct bt_le_scan_param scan_param = {
        .type = BT_SCAN_TYPE_SCAN_ACTIVE,
        .options = BT_LE_SCAN_OPT_FILTER_DUPLICATE,
        .interval = 0x00A0, // N * 0.625
        .window = 0x0014,}; // N * 0.625
