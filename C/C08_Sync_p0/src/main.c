...
void sync_pross(struct bt_conn *conn, uint8_t err, struct bt_gatt_write_params *params)
{
    ...
    // Connection interval to us
    uint32_t ci_us = conn_interv_N * 1250;

    // First response -> Save time & Send command to sync (time & n sample)
    if(tpn[idx] == 0) {
        tpn[idx] = t_ack_us;                        // save the time
        ts_us = 1000000 / fs_hz;                    // Sampling period in us

        if (!first_p) {
            // if is the first peripheral to connect
            n_sample = 1;
            start_delay_us = ts_us + ci_us - delta_us; // To star in sample 1
        } else {
            // Sync with first peripheral (P0)!!!
            // What is the next next sample that P0 will take?
            // 1. Calculate n (the next sample)
            uint32_t t_rxapp = t_ack_us + ci_us + ci_us;
            n_sample = (t_rxapp - t_ref + ts_us - 1) / ts_us;
            ...
            //Respecto al periodo de muestreo
            uint32_t ss = n_sample * ts_us;
            // Respecto al tiempo local del central
            start_delay_us = t_ref + ss;
            // Respecto al tiempo local del periferico!!!
            // Si todo sale bien, el periferico debe ejecutar el comando en tack + 2CI
            start_delay_us = start_delay_us - t_rxapp;
        }
        // Build the cmd pkt
        // uint8_t cmd_tx[12] = {0};
        ...
        LOG_INF("P[%d]: Tref %uus, Tack %us, Ts %uus, Sd %uus, sxp %u, ns %u, d %uus", idx, 
            t_ref, t_ack_us, ts_us, start_delay_us, samples_x_pkt, n_sample, delta_us);
    } else {
        // Second response
        if (t_ack_us - tpn[idx] < tol_us + 2 * ci_us) {
            ...
            // At least the first peripheral has to be sync -> Disconnect and retry
            if (!first_p) {bt_conn_disconnect(conn, BT_HCI_ERR_REMOTE_USER_TERM_CONN);}
...
static uint8_t on_gatt_notify(struct bt_conn *conn, struct bt_gatt_subscribe_params *params, const void *data, uint16_t length)
{
    ...
    if (sync_protocol && length == 1 && b[0] == 0x00){
        ...
        my_imus_write_command(conn, imus_client, &(const uint8_t){0x28}, 1, sync_pross);
    }
