/*
FALLO!!!
Segun las pruebas, y el analisis de Claud...
Para utilizar la comunicacion por usb - serial port se hace por el cable y con USB CDC
Se puede usar el UART Async ... pero requiere conexiones fisicas a los pines
POR ESO NO FUNCIONA!

De todas formas, el intentar habilitar eso consumio +2 dias asi que se descarta

La idea era usar el DMA para que se manejara aparte y en paralelo la comunicacion con la PC
con el fin de liberar la mayor cantidad de recursos para los procesos de BLE.
...
// ===================================================================
// COMUNICACIÓN UART - ASYNC MODE
// ===================================================================
#define UART_DEVICE_NODE DT_CHOSEN(zephyr_console)
#define UART_RX_BUF_SIZE 128
#define UART_TX_BUF_SIZE 255

// Buffers para DMA (deben ser estáticos)
static uint8_t uart_rx_buf0[UART_RX_BUF_SIZE];
static uint8_t uart_rx_buf1[UART_RX_BUF_SIZE];
static uint8_t uart_tx_buf[UART_TX_BUF_SIZE];

static const struct device *uart_dev = DEVICE_DT_GET(UART_DEVICE_NODE);

// Mutex para proteger TX UART
...
static uint16_t scan_window_N = 20, scan_interval_N = 160; // time(ms) = N x 0.625
...
static uint16_t conn_interv_N = 10, latency = 0, timeout_ms = 1000; // CI=Nx1.25ms
...
// ===================================================================
// FUNCIONES UART ASYNC
// ===================================================================

// Callback function to receive data via UART-SerialPort
static void uart_async_callback(const struct device *dev, struct uart_event *evt, void *user_data)
{
    static uint8_t *current_buf = uart_rx_buf0;
    static uint8_t *next_buf = uart_rx_buf1;
    
    switch (evt->type) {
        case UART_RX_RDY:
            // DMA ha transferido datos al buffer
            for (int i = 0; i < evt->data.rx.len; i++) {
                uint8_t c = evt->data.rx.buf[evt->data.rx.offset + i];
                
                // State machie to build the packet
                switch (pkt_builder.state) {
                    ...
                    case STATE_READ_END:
                        if (c == PKT_END_BYTE) {
                            // Apply the request action/command
                            handle_uart_command(pkt_builder.type, pkt_builder.buffer, pkt_builder.len);
                        } else {
        ...
        case UART_RX_BUF_REQUEST:
            ...
            uint8_t *temp = current_buf;
            current_buf = next_buf;
            next_buf = temp;
            
            int err = uart_rx_buf_rsp(dev, next_buf, UART_RX_BUF_SIZE);
            if (err) {
                LOG_ERR("UART rx_buf_rsp failed: %d", err);
            }
            break;
        ...
        case UART_RX_DISABLED:
            ...
            uart_rx_enable(dev, uart_rx_buf0, UART_RX_BUF_SIZE, 200000);
            break;
...
// Estructura del paquete: [START][LEN][TIPO][IDX][PAYLOAD][END]
void send_uart_packet(uint8_t msg_type, uint8_t p_idx, const uint8_t *payload, uint16_t len)
{
    ...
    switch (cmd_type) {
        ...
        case CMD_UPDATE_PARAMS:
            ...
            if (len == 11) {
                ...
                uint16_t cal_tout = DIV_ROUND_UP(((1 + latency) * conn_interv_N * 5), 4);
                ...
                if (print_log) printk("[C] Update PHY=%s, CI=%.2fms (%u), L=%u, STO=%ums, DL=%u, T=%uus.\n",
                    phy_2M ? "2M" : "1M", conn_interv_N*1.25, conn_interv_N, latency, timeout_ms, 
...
int main(void)
{
    ...
    if (!device_is_ready(uart_dev)) {
        ...
        printk("[INFO] UART Async API: ENABLED\n");
    #else
        printk("[ERROR] UART Async API: DISABLED - Recompile with CONFIG_UART_ASYNC_API=y\n");
        return 0;
    #endif

    err = uart_callback_set(uart_dev, uart_async_callback, NULL);
    if (err) {
        printk("[ERROR] uart_callback_set failed: %d\n", err);
        return 0;
    }

    err = uart_rx_enable(uart_dev, uart_rx_buf0, UART_RX_BUF_SIZE, 200000);
    if (err) {
        printk("[ERROR] uart_rx_enable failed: %d\n", err);
        return 0;
    }
    ...
    struct bt_le_conn_param *sc_conn_param = BT_LE_CONN_PARAM(
        conn_interv_N, conn_interv_N, latency, timeout_ms / 10);
