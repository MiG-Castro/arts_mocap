...
static uint16_t data_length = 60, tx_time_us = 500; // DL = desired value + 7B of headers
...
void send_uart_byte(uint8_t byte) {
    ...
    if (byte == PKT_START_BYTE || byte == PKT_END_BYTE || byte == PKT_ESC_BYTE) {
        ...
        uart_poll_out(uart_dev, byte ^ PKT_XOR_BYTE);        // Enviar byte modificado
    } else {
