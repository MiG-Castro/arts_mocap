...
PERIFERICO ADOPTA UN ROL PASIVO Y SIGUE LAS INSTRUCCIONES DEL CENTRAL
COMUNICACION ENCRYPTADA - PARA SU USO SOLO CON NRF52840 DONGLE
...
5. Gestión y Feedback
   - ACEPTA CAMBIO DE PARAMETROS DE CONEXION SOLICITADOS POR EL CENTRAL
...
static const struct bt_data ad[] = {
	// General advertising & No compatible with BR/EDR (Bluetooth Classic)
...
//                                  FUNCIONES BLE
//================================================================================
// Función dedicada para iniciar el advertising.
...
// Handler para este work item. Su única tarea es llamar a start_advertising.
static void adv_restart_work_handler(struct k_work *work){start_advertising();}

/************************ Connection events callback functions ************************/
// Callback para saber cuándo se completó el cambio de seguridad
static void security_changed(struct bt_conn *conn, bt_security_t level, enum bt_security_err err)
{
    if (!err) {
        LOG_INF("Link secured. Level %u.", level);
    } else {
...
// Callback a llamar cuando el BLE este listo
static void bt_ready(int err)
{
    ...
    // !!! BORRAR CLAVES DE EMPAREJAMIENTO ANTERIORES !!!
    // SOLO PARA DEPURACIÓN. Quitar en producción.
    // bt_unpair(BT_ID_DEFAULT, BT_ADDR_LE_ANY);
    // LOG_INF("Cleared all bonding information.");
	// k_msleep(200);

	// Inicializa el subsistema de settings y carga la configuración de la flash
	...
	// Ahora que BT está listo, podemos inicializar nuestro servicio y empezar a anunciar
	err = my_imus_init(&app_callbacks);
	if (err) {
		LOG_ERR("Fail to init IMU Service (err %d)", err);
		return;
	}

	// Iniciar advertising. No es necesario el k_work aquí
	start_advertising();

	// Obtener y mostrar la dirección BLE
	bt_addr_le_t addr;
	size_t count = 1;
	char addr_str[BT_ADDR_LE_STR_LEN];
	// Usamos bt_id_get que es seguro llamar después de que bt_enable ha completado.
...
static void auth_cancel(struct bt_conn *conn)
{
	...
	LOG_WRN("Pairing cancelled for %s", addr);
}

// Estructura de callbacks para "Just Works"
