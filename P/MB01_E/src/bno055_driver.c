...
bool bno055_calibration_offsets(bool write, int16_t *offsets)
{
    ...
    if (write) {
        ...
        success = true;
        if (i2c_burst_write(i2c_dev, BNO_DEFAULT_ADDR, BNO_REG_ACC_OFFSET_X_LSB, buffer, 22) != 0) success = false;
        k_msleep(50);
        // if (i2c_burst_write(i2c_dev, BNO_DEFAULT_ADDR, BNO_REG_ACC_OFFSET_X_LSB, &buffer[6], 6) != 0) success = false;
        // k_msleep(50);
        // if (i2c_burst_write(i2c_dev, BNO_DEFAULT_ADDR, BNO_REG_ACC_OFFSET_X_LSB, &buffer[12], 6) != 0) success = false;
        // k_msleep(50);
        // if (i2c_burst_write(i2c_dev, BNO_DEFAULT_ADDR, BNO_REG_ACC_OFFSET_X_LSB, &buffer[18], 2) != 0) success = false;
        // k_msleep(50);
        // if (i2c_burst_write(i2c_dev, BNO_DEFAULT_ADDR, BNO_REG_ACC_OFFSET_X_LSB, &buffer[20], 2) != 0) success = false;
        // k_msleep(50);

        if (success) LOG_INF("Calibration offsets load OK.");
        else LOG_ERR("Calibration offsets load FAILED.");
