...
// static int8_t try_connP = 0, triesUp = 5;	// Intentos de negociacion ConnP, Maximo de intentos
...
#define SAMPLING_PERIOD_US 15000 // 66.6Hz
#define START_DELAY_MS 10000	// Delay before starting the sampling
...
int main(void)
{
	...
	if (!bno055_init(i2c1_dev, BNO_OPR_MODE_IMU)) {
		...
		// Load calibration offsets - Axyz, Mxyz, Gxyz, AccRad, MagRad
		// int16_t calib_offsets[11] = {0};
		// bno055_calibration_offsets(false, calib_offsets);
		// for (int k=0; k<11 ;k++) LOG_INF("%d-%d", k, calib_offsets[k]);
		
		// Read config values (the units are important)
