/*
- Adaptación a los nuevos comandos (No se uso)
- Uso de un solo comando para el timer!!
  - Timer 1 prende led
  - Timer 2 apaga el led
*/
...
static int fs_hz, ts_us, start_delay_ms, sync_delay_us = 0;
...
static void app_command_cb(const uint8_t *buf, uint16_t len)
{
	...
	if (buf[0] == 0x24) {
		...
		// Start the timer
		k_timer_start(&sensor_timer, K_USEC(delay_us), K_USEC(50000));
		k_timer_start(&sensor_timer_off, K_USEC(delay_us + 8000), K_USEC(50000));
		LOG_INF("%u", delay_us);
	}
