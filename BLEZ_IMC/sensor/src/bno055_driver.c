/* Made by Miguel Angel Castro-Perez - 2025
   ...
   For an unknown reason (and contrary to what the datasheet says)
   1. The BNO doesn't reset on power on, so you need to reset it manually.
   2. The BNO needs a "big" delay after reset to start working correctly, I recommend 800ms.
   3. The BNO needs a "big" delay after setting the external crystal, even if the system clock status is ok ... I recommend 650ms.
...
// Static pointer to the I2C device, initialized in bno055_init
static const struct device *i2c_dev;
#define BNO_CONFIG_DELAY_MS 25 // Small delay after changing mode
...
{
    i2c_dev = i2c_dev_node; // Assign the I2C device pointer
    if (!device_is_ready(i2c_dev)) {
        LOG_ERR("I2C device not ready.");
        return false;
    }

    // DELAY TO ALLOW THE BNO TO INITIALIZE
    k_msleep(800);
    uint8_t chip_id = 0;
...
bool bno055_change_crystal(bool set_external_crystal)
{
    ...
    // Verify the crystal state after changing it
...
{
    float div = 1.0f;
    int stop = 3; // 3 axes (X,Y,Z) by default
