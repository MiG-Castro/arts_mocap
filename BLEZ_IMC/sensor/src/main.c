...
static void sensor_work_handler(struct k_work *work)
{
	...
	*/
	...
	// store the sample according to the temporal counter
	// 4B offset because the Packet Number (NoPkts)
...
int main(void)
{
	...
	// Init the BNO and select the operation mode.
	// Note, if the NDOF mode is unstable change to IMU mode (caution in this mode mag is disable)
	if (!bno055_init(i2c1_dev, BNO_OPR_MODE_NDOF)) {
		...
		// This can help if the fusion algorithm is unstable
		int16_t bno_cal_offsets[11] = {-3, 5, -34, 32325, -738, 10118, -1, -2, 3, 1000, 773};
