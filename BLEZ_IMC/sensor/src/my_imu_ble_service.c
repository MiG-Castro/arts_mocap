...
static void ccc_sensordata_cfg_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
	...
	LOG_INF("Subscription to SensorData %s", sensordata_notify_enabled ? "enable" : "disable");
}
...
static void ccc_exercisedetection_cfg_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
	...
	LOG_INF("Subscription to ExerciseDetection %s", exercisedetection_indicate_enabled ? "enable" : "disable");
}
...
{if (err != 0) LOG_WRN("ExerciseDetection indication failed (err %u)", err);}

/*
 * IMU Service Declaration (Order: SensorData, ExerciseDetection, Command)
 */
...
int my_imus_send_exercisedetection(const uint8_t *data, uint16_t len)
{
	...
	// ExerciseDetection attribute: Index 4
...
{return exercisedetection_indicate_enabled;}
