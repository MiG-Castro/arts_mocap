/*
 * BLEZ - Central Node Firmware
 * Author: Miguel Angel Castro-Perez, 2026
*/
...
#define UART_RX_BUF_SIZE 500 // 255
#define UART_TX_BUF_SIZE 500 // 100
...
// COMMANDS
// To the peripherals
#define CMD_TYPE_SEND_CMD    0x00
#define CMD_CTRL_NOTIFY      0x01
#define CMD_CTRL_INDICATE    0x02
#define CMD_START_STOP_TX    0x03
#define CMD_STOP_PKT         0x04
#define CMD_FS               0x05
#define CMD_SAMP_X_PKT       0x06
// To the central
...
#define CMD_TYPE_START_SCAN  0x1A
#define CMD_TYPE_STOP_SCAN   0x1B
#define CMD_TYPE_DISCONNECT  0x1C
#define CMD_UPDATE_PARAMS    0x1D
// Pkt info
...
static uint16_t scan_window_N = 24, scan_interval_N = 240; // units of 0.625ms
...
static uint16_t conn_interv_N = 12, latency_N = 10, timeout_N = 100;
// Values to set in the data length
static uint16_t data_length = 251, tx_time_us = 1095; // DL = desired value
...
static const bt_addr_le_t sensor_wl[] = {
	// To add the address in the list, invert the sequence of the numbers
	// Example FB:5C:2A:77:77:0A -> .a = {.val = {0x0A, 0x77, 0x77, 0x2A, 0x5C, 0xFB}}
...
// Print/log and UART packet transmission
...
{
	if (print_log) printk(" - Data Length (Tx/Rx): %u/%u bytes, MRT(Tx/Rx): %u/%u us.\n", 
...
static void on_conn_param_updated(struct bt_conn *conn, uint16_t interval, uint16_t latency, uint16_t timeout)
{
    ...
    double pl = latency * ci;
	uint16_t sto = timeout * 10;
	if (print_log) printk(" - Connection parameters set to %.2f ms (CI), %.2f ms (PL), %d ms (ST).\n", ci, pl, sto);
...
static void handle_uart_command(const uint8_t *pkt, uint8_t lnn)
{
    ...
    switch (cmd_type) {
        ...
        case CMD_UPDATE_PARAMS:
            ...
            // PHY:1B, CI:2B, L:2B, ST:2B(ms), DL:2B, Tx/Rx:2B(us) = 11B
            if (len == 11) {
                uint16_t n;
                ...
                // 4. Supervision timeout (greater than latency)
